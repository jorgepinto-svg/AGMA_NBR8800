...
\section{Introdução}
Esta memória de cálculo tem como objetivo demonstrar a verificação completa do perfil metálico chamado \textit{PM-009}, a qual foi 
...
\begin{tabular}{|c||c|c|c|c|c|c|}
    \hline
        ...
        PM-009   & W200X52 &   206.0   &   204.0     &    12.6     &    7.9    &    66.5  \\ \hline 
\end{tabular}
\end{center}
\end{table}
...
PM-009 & W200X52 &   32.0  &     5300.0   &   1800.0    &   16.2        &   42.8           \\ \hline 
\end{tabular}
\end{center}
\end{table}
...
PM-009 & W200X52 &      514.0      &     174.0     &     569.0      &      264.0     &    51.591    &     89.19    \\ \hline 
\end{tabular}
\end{center}
\end{table}
...
Para esta análise foi escolhido o ponto localizado a $0m$ do ponto inicial da barra \textit{PM-009}. Ao avaliar
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{1cm}||p{2.5cm}|p{1.1cm}|p{1.1cm}|p{1.1cm}|p{1.1cm}|p{1.1cm}|p{1.1cm}|}
        \hline
            ...
            PM-009 & ELU 001    &      -295.18    &      0    &      0    &       0     &       0       &       0        \\ \hline 
    \end{tabular}
    \end{center}
    \end{table}
...
$$M_r = (f_y-\sigma_r).W_x = 0.7 * (344.7379*10^6) * (514.0*10^{-6}) = 124.0kN.m$$\\ 
\\
...
$$M_{cr} = \frac{C_b. \pi^2.E.I_x}{L_b^2}\sqrt{\frac{C_w}{I_x}(1+0.039.\frac{JL_b^2}{C_w})} = $$
$$M_{cr} = \frac{1 * 3.1415^2 * (199.948*10^9) * (5300.0*10^{-8}) }{ (1*3)^2}*\sqrt{\frac{1.68316*10^{-7}}{5300.0*10^{-8}}*(1 + 0.039 * \frac{(32.0*10^{-8})*(1*3)^2}{1.68316*10^{-7}})}$$\\
$$M_{cr} = 845.6kN.m$$\\
\\
...
$$\lambda = \frac{L_b}{r_x} = \frac{1*3}{0.0892} = 33.64$$\\
\\
...
Sendo que:\\
$$\beta_1 = \frac{(f_y-\sigma_r).W_x}{E.J} = \frac{ 0.7 * (344.7379*10^6) * (514.0*10^{-6}) }{ (199.948*10^9) * (32.0*10^{-8}) } = 1.939$$\\

Assim, aplicando o valor de $\beta_1$ obtido, têm-se:

$$\lambda_r = \frac{1.38 * 1*\sqrt{ (5300.0*10^{-8}) * (32.0*10^{-8}) } }{ 0.0892 * (32.0*10^{-8}) * 1.939 }. \sqrt{ 1 + \sqrt{ 1 + \frac{ 27 * (1.68316*10^{-7}) * 1.939^2 }{ 1^2 * (5300.0*10^{-8}) }}} = 150.61 $$\\
\\
...
$$M_{pl}= Z_x.f_y = (569.0*10^{-6}) * (344.7379*10^6) = 196.2kN.m$$
...
\\
Como: $\lambda = 33.64$; $\lambda_p = 42.39$ e $\lambda_r = 150.61$.\\
Então $\lambda \leq \lambda_p$.
\\
Sendo assim: 
$$M_{Rd} = \frac{1}{\gamma_{a1}}M_{pl} = \frac{1}{1.1}*196.2 = 178.3kN.m$$
\\
...
$$W_c = \frac{\frac{0.204*0.0126^3}{12} + (0.204*0.0126).(\frac{0.206}{2}-\frac{0.0126}{2})^2 + \frac{0.0079.(\frac{0.206}{2} - 0.0126)^3}{3}}{\frac{0.206}{2}} = 252.5*10^{-6}m^3 $$


Assim, aplicando os valores de $W_c$ e $\lambda$ obtidos, têm-se:

$$M_{cr} = \frac{0.69.E}{\lambda^2}W_c = \frac{ 0.69 * (199.948*10^9) }{ (\frac{ 0.204/2 }{ 0.0126 })^2}*(252.5*10^{-6}) = 531.6kN.m$$\\
\\
...
$$\lambda = \frac{b}{t} = \frac{0.204/2}{0.0126} = 8.1$$\\
\\
...
$$M_{pl}= Z.f_y = (569.0*10^{-6}) * (344.7379*10^6) = 196.2kN.m$$\\
\\
...
\\
Como: $\lambda = 8.1$; $\lambda_p = 9.15$ e $\lambda_r = 23.89$.\\
Então $\lambda \leq \lambda_p$.
\\
Sendo assim: 
$$M_{Rd} = \frac{1}{\gamma_{a1}}M_{pl} = \frac{1}{1.1}*196.2 = 178.3kN.m$$
...
$$M_r = f_y.W_x = (344.7379*10^6) * (514.0*10^{-6}) = 177.2kN.m$$\\ 
\\
...
$$\lambda = \frac{h}{t_w} = \frac{0.206-0.0126-0.0126}{0.0079} = 22.97$$\\
\\
Onde:\\
...
\\
Como: $\lambda = 22.97$; $\lambda_p = 90.55$ e $\lambda_r = 137.27$.\\
Então $\lambda \leq \lambda_p$.
\\
Sendo assim: 
$$M_{Rd} = \frac{1}{\gamma_{a1}}M_{pl} = \frac{1}{1.1}*196.2 = 178.3kN.m$$
...
Como os valores são: \\
$178.3 kN.m$ para Momento Fletor Resistente de Cálculo segundo FLT;\\
$178.3 kN.m$ para Momento Fletor Resistente de Cálculo segundo FLM;\\
$178.3 kN.m$ para Momento Fletor Resistente de Cálculo segundo FLA.\\


Então o Momento Fletor Resistente de Cálculo para o eixo de maior inércia é $178.3 kN.m$.\\
...
Sendo que a tensão residual de compressão nas mesas, $\sigma_r$, deve ser considerada igual 
a 30\% da resistência ao escoamento do aço utilizado.
$$M_r = (f_y-\sigma_r).W_y = 0.7 * (344.7379*10^6) * (174.0*10^{-6}) = 42.0kN.m$$\\ 
\\
...
$$W_c = \frac{0.0126*(\frac{0.204}{2})^3}{3} + \frac{(0.206-2*0.0126).(\frac{0.0079}{2})^3}{3} + \frac{0.0126.(\frac{0.204}{2})^3}{3} = 87.4*10^{-6}m^3
\\
\\

Assim, aplicando os valores de $W_c$ e $\lambda$ obtidos, têm-se:

$$M_{cr} = \frac{0.69.E}{\lambda^2}W_c = \frac{ 0.69 * (199.948*10^9) }{ ( \frac{ 0.204/2 }{ 0.0126 })^2} * (87.4*10^{-6}) = 184.1kN.m$$\\
\\
...
$$M_{pl}= Z.f_y = (264.0*10^{-6}) * (344.7379*10^6) = 91.0kN.m $$\\
...
\\
Sendo assim: 
$$M_{Rd} = \frac{1}{\gamma_{a1}}M_{pl} = \frac{1}{1.1}*91.0 = 82.7kN.m$$
\\
...
\\
$$V_{pl} = 0.6.A_w.f_y = 0.6 * (0.206* 0.0079) * (344.7379*10^6) = 335.3 kN$$
\\
\\



Como $\lambda = 22.97$ ; $\lambda_p = 61.22$ e $\lambda_r = 76.24$.
\\
Então $\lambda \leq \lambda_p$:
$$V_{Rd} = \frac{V_pl}{\gamma_{a1}} = \frac{335.3}{1.1} = 304.9kN$$
\\
...
$$\lambda = \frac{h}{t_w} = \frac{0.204/2}{0.0126} = 8.1$$
\\
...
\\
$$V_{pl} = 0.6.A_w.f_y = 0.6 * 2 * (0.204* 0.0126) * (344.7379*10^6) = 1063.3kN$$
\\
\\



Como $\lambda = 8.1$ ; $\lambda_p = 29.02$ e $\lambda_r = 36.14$.
\\
Então $\lambda \leq \lambda_p$:
$$V_{Rd} = \frac{V_pl}{\gamma_{a1}} = \frac{1063.3}{1.1} = 966.7kN$$
\\
...
$$\frac{-295.18}{2*1613.1}+ \left(\frac{0}{178.3}+\frac{0}{82.7}\right) \leq 1.0$$

\vspace{0.3cm}

$$0.183/2+ \left(0+0\right) \leq 1.0$$
...
Sendo assim, $V_{y,Sd} = 0$ e $V_{y,Rd} = 304.9$ são os esforços cortantes que atuam na direção paralela a alma do perfil.\\
\vspace{0.3cm}
\\



A relação $V_{y,Sd} \leq V_{y,Rd}$ é atendida e a taxa de aproveitamento do perfil (razão $V_{y,Sd} / V_{y,Rd}$) é igual a 0.\\
\\

   

Sendo assim, $V_{x,Sd} = 0$ e $V_{x,Rd} = 966.7$ são os esforços cortantes que atuam na direção transversal a alma do perfil.\\
\vspace{0.3cm}
\\



A relação $V_{x,Sd} \leq V_{x,Rd}$ é atendida e a taxa de aproveitamento do perfil (razão $V_{x,Sd} / V_{x,Rd}$) é igual a 0.\\
\\

   

\section{Conclusão}
O perfil metálico chamado \textit{PM-009}, a qual foi adotada a seção \textit{W200X52}, e na posição 0m encontra-se:\\
\\
